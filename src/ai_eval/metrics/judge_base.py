"""Shared base for all LLM-as-a-judge metrics.

Handles caching (thread-safe), prompt template substitution, and JSON
parsing so individual metrics only declare their template and rubric.
"""
...
CACHE_PATH = Path.home() / ".cache" / "ai_eval" / "judge_cache.json"
...
class JudgeBase(Metric):
    """Base class for judge metrics. Subclasses set:
      - name
      - threshold
      - rubric_version
      - prompt_template (str with __FIELD__ placeholders)
      - score_key (JSON key containing the numeric score)
    """
    ...
    def build_prompt(self, trace: Trace) -> str:
        """Subclasses override if they need custom substitution."""
    ...
    def measure(self, trace: Trace) -> Score:
        ...
        try:
            ...
            return Score(
                ...
                evidence=f"JUDGE_ERROR: {type(e).__name__}: {truncate(str(e), 150)}",
                judge_model=self.model,
                judge_prompt_hash=self.prompt_hash,
                rubric_version=self.rubric_version,
